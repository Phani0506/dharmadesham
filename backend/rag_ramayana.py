...
def load_data():
    ...
    if _index is None:
        if os.path.exists("ramayana.index"):
            _index = faiss.read_index("ramayana.index")
        else:
            raise Exception("FAISS index not found. Run ingest_ramayana.py first.")
            
    if _metadata is None:
        if os.path.exists("ramayana_meta.pkl"):
            with open("ramayana_meta.pkl", "rb") as f:
                _metadata = pickle.load(f)
        else:
            raise Exception("Metadata not found. Run ingest_ramayana.py first.")
...
def generate_answer(query: str, retrieved_verses: list):
    ...
    for i, v in enumerate(retrieved_verses):
        ...
        context_str += f"Kanda: {v.get('kanda')}, Sarga: {v.get('sarga')}, Shloka: {v.get('shloka_num')}\n"
        ...
        context_str += f"Explanation: {v.get('explanation')}\n\n"
        
    system_instruction = """You are a polite, authoritative Vedic scholar.
Your task is to answer user questions about the Ramayana based ONLY on the provided retrieved verses.
